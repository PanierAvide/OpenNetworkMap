...
/**
 * Exports {@link Canvas} as SVG images.
 */
public class CanvasToSVGExporter implements CanvasExporter {
//CONSTANTS
...
//OTHER METHODS
	...
	public boolean export(Canvas c, File output) throws IOException {
		 ...
		 */
		//Canvas style
		renderCanvas(c, svgGenerator);
		
		//Render edges and nodes
		renderEdges(c, svgGenerator);
		renderNodes(c, svgGenerator);
...
//		}
	 ...
	 * @param generator The graphics handler
	 */
	private void renderNodes(Canvas c, Graphics2D generator) {
		...
		for(Vertex v : nodesPos.keySet()) {
			...
			int x = pos[1]*nodeMargins - nodeWidth/2;
			int y = -pos[0]*nodeMargins - nodeWidth/2;
			switch(nodeShape) {
				case "circle":
	 ...
	 * @param generator The graphics handler
	 */
	private void renderEdges(Canvas c, Graphics2D generator) {
		...
		for(Edge e : edgesPos.keySet()) {
			 ...
			 * Draw edge
			 */
			int x1 = pos[1]*nodeMargins;
			int y1 = -pos[0]*nodeMargins;
			int x2 = pos[3]*nodeMargins;
			int y2 = -pos[2]*nodeMargins;
...
//				generator.translate((x2 - x1)/2 + x1 + offsetLabelX, (y2 - y1)/2 + y1 + offsetLabelY);
				...
				double angle = c.getEdgeDirection(e) % (2*Math.PI);
...
//INTERN CLASS Couple
	...
	//OTHER METHODS
		...
		private CanvasToSVGExporter getOuterType() {
			return CanvasToSVGExporter.this;
		}
	}
}
